"""Tests for `skills/formatos/scripts/format_report.py`.
...
# Import the format_report module (lives in a scripts dir without a parent
...
import format_report  # type: ignore  # noqa: E402
...
class TestFormatDate:
    def test_public_date_takes_precedence(self):
        ...
        assert format_report._format_date(job, analysis) == "2026-06-20"

    def test_falls_back_to_created_at(self):
        job = {"public_date": None}
        analysis = {"created_at": "2026-06-25 10:00:00"}
        assert format_report._format_date(job, analysis) == "2026-06-25 10:00:00"

    def test_both_missing_returns_empty(self):
        assert format_report._format_date({}, {}) == ""


class TestFormatComparativa:
    def test_normalizes_to_bullets(self):
        out = format_report._format_comparativa("X | an\r\nY | bn")
        assert out == "- X | an\n- Y | bn"

    def test_preserves_literal_pipe_separator(self):
        out = format_report._format_comparativa("Requisito | Análisis: eval con | pipe")
        assert "- Requisito | Análisis: eval con | pipe" in out

    def test_already_bulleted(self):
        out = format_report._format_comparativa("- A | x\n- B | y")
        assert out == "- A | x\n- B | y"

    def test_empty_falls_back(self):
        assert format_report._format_comparativa("") == "_(Sin comparativa)_"
        assert format_report._format_comparativa(None) == "_(Sin comparativa)_"

    def test_blank_lines_skipped(self):
        out = format_report._format_comparativa("a | 1\n\n\nb | 2")
...
class TestLinksHtml:
    def test_includes_available_links(self):
        ...
        out = format_report._links_html(profile)
    ...
    def test_missing_links_omitted(self):
        out = format_report._links_html({"cv_url": None, "linkedin": None, "github": None})
        assert out == ""
...
class TestBuildMarkdown:
    def test_complete_report_all_sections(self, tmp_db):
        ...
        md = format_report._build_markdown(job, analysis, {"cv_url": None, "linkedin": None, "github": None})
    ...
    def test_all_optional_missing_degrades_gracefully(self, tmp_db):
        # url/public_date are Optional in the model -> real None from DB; the
        # required str analysis fields are seeded empty to simulate "absent".
        ...
        # date falls back to created_at (no public_date)
        assert analysis["created_at"] in md
    ...
    def test_profile_links_appended(self, tmp_path, tmp_db):
        ...
        job, analysis = _load(h)
        profile = {"cv_url": "https://cv", "linkedin": "https://li", "github": "https://gh"}
        md = format_report._build_markdown(job, analysis, profile)
    ...
    def test_no_profile_links_no_section(self, tmp_db):
        h = _seed_job()  # URL present -> Fuente contains an anchor, that's fine
        _seed_analysis(h)
        job, analysis = _load(h)
        md = format_report._build_markdown(job, analysis, {"cv_url": None, "linkedin": None, "github": None})
        # No *profile* links footer (CV/LinkedIn/GitHub labels) — but the Fuente
        # anchor from the job URL is still legitimate.
...
class TestBuildJson:
    def test_includes_all_spec_keys(self, tmp_db):
        ...
        _seed_analysis(h)
        job, analysis = _load(h)
        obj = format_report._build_json(job, analysis, {"cv_url": None, "linkedin": None, "github": None})
        required = {
    ...
    def test_null_fields_are_json_null(self):
        # The required-str analysis fields cannot be None via the validated
        # model insert path, but the builder must still emit JSON null (not the
        # strings "null"/"None") wherever the DB row actually holds null.
        ...
        obj = format_report._build_json(job_row, analysis_row, {})
...
class TestCLI:
    ...
    def test_default_markdown(self, tmp_db, tmp_path, monkeypatch):
        ...
        assert "Apto" in out
        # profile links included (perfil has all three)
    ...
    def test_missing_job_flag_is_usage_error(self, tmp_db):
        ...
        # Typer exits non-zero (2) when a required option is missing and prints
        # usage help mentioning --job.
        assert result.exit_code != 0
    ...
    def test_markdown_without_profile_links(self, tmp_db, tmp_path, monkeypatch):
        ...
        result = runner.invoke(format_report.app, ["--job", h])
        assert result.exit_code == 0, result.stderr
        # No profile-footer anchors — the job Fuente anchor is unrelated.
