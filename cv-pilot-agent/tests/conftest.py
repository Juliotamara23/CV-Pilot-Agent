"""Shared fixtures for CV-Pilot tests.
...
# Put cv-pilot-agent/ on sys.path so `import _lib` works from the test process.
_AGENT_ROOT = Path(__file__).resolve().parent.parent
if str(_AGENT_ROOT) not in sys.path:
    sys.path.insert(0, str(_AGENT_ROOT))

# DDL mirrors scripts/init.py verbatim (which is code-guard protected and uses a
